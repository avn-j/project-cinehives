...
export default function MovieCard({
  ...
  const [liked, setLiked] = useState(userActivity.includes("LIKE") || false);
  ...
  const [rated, setRated] = useState(userActivity.includes("RATING") || false);
    ...
    <div className="">
      <HoverCard openDelay={0} closeDelay={0}>
        <HoverCardTrigger asChild>
          <Link href={`/${media.mediaType.toLowerCase()}/${media.apiId}`}>
            <Image
      ...
      {otherUserRatingActivity && (
        <UserRating
          ...
          profilePictureSrc={otherUserRatingActivity.profilePictureURL}
        />
      )}
    </div>
  );
